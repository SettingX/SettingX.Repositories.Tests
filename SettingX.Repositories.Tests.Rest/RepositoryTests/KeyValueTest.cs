...
{
    [DefaultPriority(0)]
    public class KeyValueTest : IClassFixture<ClientFixture>
    {
        private readonly IClient _client;

        public KeyValueTest(ClientFixture fixture)
        {
        ...
        public async Task Write_Read_Update_Read()
        {
            ...
            await _client.KeyValuesApi.UpdateKeyValueAsync(new List<KeyValue> {keyValue});

            var keyValueFromDb1 = await _client.KeyValuesApi.GetKeyValueAsync(keyValue.KeyValueId);
            var keyValueFromDb2 = await _client.KeyValuesApi.GetKeyValuesAsync(new List<string> {keyValue.KeyValueId});
            ...
            await _client.KeyValuesApi.ReplaceKeyValueAsync(new List<KeyValue> {keyValue});
            
            keyValueFromDb1 = await _client.KeyValuesApi.GetKeyValueAsync(keyValue.KeyValueId);
            keyValueFromDb2 = await _client.KeyValuesApi.GetKeyValuesAsync(new List<string> {keyValue.KeyValueId});
        ...
        public async Task Write_Find()
        {
            ...
            var keyValueFromDb = await _client.KeyValuesApi.GetAsync(null, keyValue.Types[0]);
            
            Assert.True(Equal(keyValue, keyValueFromDb.Single()));
            Assert.Single(keyValueFromDb);
            
            keyValueFromDb = await _client.KeyValuesApi.GetKeyValuesAsync(keyValue.RepositoryNames[0], null, null, null);
            
            Assert.True(Equal(keyValue, keyValueFromDb.Single()));
            Assert.Single(keyValueFromDb);
            
            keyValueFromDb = await _client.KeyValuesApi.GetKeyValuesAsync(null, null, null, keyValue.RepositoryId);
            
            Assert.True(Equal(keyValue, keyValueFromDb.Single()));
            Assert.Single(keyValueFromDb);
            
            keyValueFromDb = await _client.KeyValuesApi.GetKeyValuesAsync(null, null, keyValue.Override[0].Value.Substring(15), null);
            
            Assert.Contains(keyValueFromDb, x => Equal(keyValue, x));
            
            keyValueFromDb = await _client.KeyValuesApi.GetKeyValuesAsync(null, keyValue.RepositoryNames[0], keyValue.Override[0].Value.Substring(15), null);
            
            Assert.True(Equal(keyValue, keyValueFromDb.Single()));
            Assert.Single(keyValueFromDb);
            
            keyValueFromDb = await _client.KeyValuesApi.GetKeyValuesAsync(null, keyValue.RepositoryNames[0], null, null);
            
            Assert.True(Equal(keyValue, keyValueFromDb.Single()));
            Assert.Single(keyValueFromDb);
            
            var emptySearch = await _client.KeyValuesApi.GetKeyValuesAsync(Guid.NewGuid().ToString(), null, null, keyValue.RepositoryId);
            
            Assert.Empty(emptySearch);
            
            emptySearch = await _client.KeyValuesApi.GetKeyValuesAsync(null, null, Guid.NewGuid().ToString(), keyValue.RepositoryId);
            
            Assert.Empty(emptySearch);
            
            emptySearch = await _client.KeyValuesApi.GetKeyValuesAsync(null, Guid.NewGuid().ToString(), null, keyValue.RepositoryId);
        ...
        public async Task Write_Remove_Read_Remove_Read()
        {
            ...
            await _client.KeyValuesApi.RemoveNetworkOverridesAsync(keyValue.Override[0].NetworkId);
            
            var keyValueFromDb = await _client.KeyValuesApi.GetAsync(null, keyValue.Types[0]);
            
            Assert.True(keyValueFromDb[0].Override.All(x => x.NetworkId != keyValue.Override[0].NetworkId));

            await _client.KeyValuesApi.DeleteKeyValueWithHistoryAsync(keyValue.KeyValueId, Guid.NewGuid().ToString(),
                Guid.NewGuid().ToString(), Guid.NewGuid().ToString());
            
            keyValueFromDb = await _client.KeyValuesApi.GetAsync(null, keyValue.Types[0]);
        ...
        {
            var all = await _client.KeyValuesApi.GetAsync();
